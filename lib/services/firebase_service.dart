...
class FirebaseService {
  ...
  // FCM Token
  Future<void> saveFcmToken(String pairId, String userId, String s) async {
  ...
  Stream<List<MoodModel>> watchMoods(String userId) {
    ...
    return _firestore.collection('moods').where('userId', isEqualTo: userId).snapshots().map((snapshot) {
      final moods = snapshot.docs.map((doc) => MoodModel.fromMap(doc.data())).toList();
  ...
  Stream<List<MoodModel>> watchMoodsForDateRange(
    ...
    return _firestore.collection('moods').where('timestamp', isGreaterThanOrEqualTo: start).snapshots().map((snapshot) {
      final moods = snapshot.docs.map((doc) => MoodModel.fromMap(doc.data())).toList();
      // Filter by end date in memory
      final filtered = moods.where((mood) => mood.timestamp.isAfter(start.subtract(const Duration(seconds: 1))) && mood.timestamp.isBefore(end.add(const Duration(seconds: 1)))).toList();
  ...
  Stream<List<WhisperModel>> watchWhispers(String userId1, String userId2) {
    ...
    return _firestore.collection('whispers').where('senderId', whereIn: [userId1, userId2]).snapshots().map((snapshot) {
  ...
  Stream<List<SealedLetterModel>> watchSealedLetters(
    ...
    // Remove orderBy to avoid index requirement, sort in memory instead
    return _firestore.collection('sealedLetters').where('senderId', whereIn: [userId1, userId2]).snapshots().map((snapshot) {
  ...
  Stream<List<PokeModel>> watchPokes(String receiverId) {
    return _firestore.collection('pokes').where('receiverId', isEqualTo: receiverId).where('isHandled', isEqualTo: false).snapshots().map((snapshot) {
